# -*- coding: utf-8 -*-
content = """/**
 * Curated Dataset for Walk Around Virtual Vacation Walk - Art by Beckman
 * Scale: 1 unit = 1.0 meter (Physical 1:1 Metric Scale)
 * 5 Photorealistic Parisian Penthouse Zones with Fredrik Beckman's Curated Paintings
 */
...
const WALKAROUND_ROOMS = {
    entry: {
        ...
        minimapPos: { x: 45, y: 105 },
        initialYaw: -0.05,
        initialPitch: 0.0,
        walkSequence: [
            { yaw: -0.05, pitch: -0.02, zoom: 1.0, duration: 3.5, label: "Entréöversikt" },
            { yaw: 0.28, pitch: 0.05, zoom: 1.25, duration: 4.5, targetArt: 271, label: "Gallerivägg — Golden Ticket & Warmpop" },
            { yaw: -0.22, pitch: -0.02, zoom: 1.35, duration: 4.0, targetPortal: "living_room", label: "Går mot portalen till Vardagsrummet" }
        ],
        artworks: [
            {
                id: 271,
                ...
                screenPos: { x: 73.2, y: 23.5, radius: 65 }
            },
            {
                id: 264,
                title: "Warmpop",
                ...
                screenPos: { x: 78.0, y: 48.5, radius: 50 }
            }
        ],
        portals: [
            {
                ...
                screenPos: { x: 26.5, y: 55.0 },
                direction: "forward"
            },
            {
                targetRoom: "bedroom",
                label: "Gå till Sovrummet",
                screenPos: { x: 92.0, y: 64.0 },
                direction: "right"
            }
        ]
    },
    ...
    living_room: {
        ...
        minimapPos: { x: 90, y: 75 },
        initialYaw: 0.0,
        initialPitch: 0.0,
        walkSequence: [
            { yaw: -0.05, pitch: -0.02, zoom: 1.0, duration: 3.5, label: "Salongsöversikt mot bouclésoffan" },
            { yaw: 0.12, pitch: 0.04, zoom: 1.30, duration: 5.0, targetArt: 265, label: "Huvudvägg — My Heart Has Teeth" },
            { yaw: 0.42, pitch: -0.02, zoom: 1.35, duration: 4.0, targetPortal: "dining_room", label: "Går mot portalen till Matsalen" }
        ],
        artworks: [
            {
                id: 265,
                ...
                screenPos: { x: 53.8, y: 30.2, radius: 95 }
            }
        ],
        portals: [
            {
                ...
                screenPos: { x: 81.5, y: 48.0 },
                direction: "forward"
            },
            {
                targetRoom: "entry",
                label: "Tillbaka till Entrén",
                screenPos: { x: 7.5, y: 72.0 },
                direction: "left"
            }
        ]
    },
    ...
    dining_room: {
        ...
        minimapPos: { x: 135, y: 55 },
        initialYaw: -0.1,
        initialPitch: 0.0,
        walkSequence: [
            { yaw: -0.10, pitch: -0.02, zoom: 1.0, duration: 3.5, label: "Matsalsöversikt med marmorbordet" },
            { yaw: -0.40, pitch: 0.05, zoom: 1.30, duration: 5.0, targetArt: 258, label: "Fondvägg — Daylight" },
            { yaw: 0.38, pitch: -0.04, zoom: 1.35, duration: 4.0, targetPortal: "kitchen", label: "Går mot passagen till Köket" }
        ],
        artworks: [
            {
                id: 258,
                ...
                screenPos: { x: 17.5, y: 25.5, radius: 90 }
            },
            {
                id: 270,
                title: "Origami",
                ...
                screenPos: { x: 87.5, y: 38.0, radius: 55 }
            }
        ],
        portals: [
            {
                targetRoom: "kitchen",
                label: "Gå in i Köket",
                screenPos: { x: 12.0, y: 68.0 },
                direction: "left"
            },
            {
                targetRoom: "living_room",
                label: "Tillbaka till Vardagsrummet",
                screenPos: { x: 74.0, y: 52.0 },
                direction: "right"
            }
        ]
    },
    ...
    kitchen: {
        ...
        minimapPos: { x: 135, y: 108 },
        initialYaw: 0.0,
        initialPitch: 0.0,
        walkSequence: [
            { yaw: 0.0, pitch: -0.02, zoom: 1.0, duration: 3.5, label: "Köksöversikt mot den skulpturala köksön" },
            { yaw: -0.42, pitch: 0.06, zoom: 1.30, duration: 5.0, targetArt: 22, label: "Fönstervägg — Chromeyellowred 1.0" },
            { yaw: 0.38, pitch: -0.03, zoom: 1.35, duration: 4.0, targetPortal: "dining_room", label: "Går mot portalen till Matsalen" }
        ],
        artworks: [
            {
                id: 22,
                ...
                screenPos: { x: 7.8, y: 32.5, radius: 70 }
            }
        ],
        portals: [
            {
                ...
                screenPos: { x: 80.0, y: 52.0 },
                direction: "forward"
            },
            {
                targetRoom: "entry",
                label: "Tillbaka mot Entrén",
                screenPos: { x: 10.0, y: 72.0 },
                direction: "left"
            }
        ]
    },
    ...
    bedroom: {
        ...
        minimapPos: { x: 45, y: 45 },
        initialYaw: 0.0,
        initialPitch: 0.0,
        walkSequence: [
            { yaw: 0.0, pitch: -0.02, zoom: 1.0, duration: 3.5, label: "Sovrumsöversikt mot bouclésängen och fönstret" },
            { yaw: -0.32, pitch: 0.08, zoom: 1.30, duration: 5.0, targetArt: 270, label: "Huvudvägg — Origami Monumental" },
            { yaw: 0.28, pitch: -0.04, zoom: 1.35, duration: 4.0, targetPortal: "entry", label: "Går mot dörren ut till Vestibulen" }
        ],
        artworks: [
            {
                id: 270,
                ...
                screenPos: { x: 21.8, y: 21.5, radius: 95 }
            }
        ],
        portals: [
            {
                ...
                screenPos: { x: 8.0, y: 68.0 },
                direction: "left"
            },
            {
                targetRoom: "living_room",
                label: "Gå till Vardagsrummet",
                screenPos: { x: 92.0, y: 65.0 },
                direction: "right"
            }
        ]
    }
...
const WALKAROUND_CURATED_ROOM = [
    WALKAROUND_ROOMS.living_room.artworks[0],
    WALKAROUND_ROOMS.dining_room.artworks[0],
    WALKAROUND_ROOMS.bedroom.artworks[0],
    WALKAROUND_ROOMS.entry.artworks[0],
    WALKAROUND_ROOMS.entry.artworks[1],
    WALKAROUND_ROOMS.kitchen.artworks[0]
];

for target in ['js/walkaround-data.js', 'dist/js/walkaround-data.js']:
